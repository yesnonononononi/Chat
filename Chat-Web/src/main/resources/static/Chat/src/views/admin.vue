<template>
    <div class="w-screen h-screen overflow-hidden box-border" :class="white_page ? 'theme-light' : 'theme-dark'">
        <div class="flex items-center w-full h-full">
            <div id="left" class="flex flex-col justify-around whitespace-nowrap   transition-all duration-300  h-full "
                :class="!isFold ? 'w-32 translate-x-0 mx-8 opacity-100' : '-translate-x-full w-0 mx-0 opacity-0'">
                <span class="text-2xl md:text-3xl text-green-500 font-bold font-glow">chat</span>
                <span class="text-sm md:text-lg  glow-white menu-item "
                    @click="router.push({ name: 'total-data' })">数据概览</span>
                <span class="text-sm md:text-lg glow-white menu-item "
                    @click="router.push({ name: 'user-admin' })">用户管理</span>
                <span class="text-sm md:text-lg  glow-white menu-item"
                    @click="router.push({ name: 'group-admin' })">群聊管理</span>
                <span class="text-sm md:text-lg glow-white menu-item"
                    @click="router.push({ name: 'system-admin' })">系统管理</span>
                <span class="text-sm md:text-lg glow-white menu-item"
                    @click="router.push({ name: 'system-log-admin' })">日志管理</span>
                <span class="text-sm md:text-lg glow-white menu-item"
                    @click="router.push({ name: 'emoji-admin' })">表情管理</span>
                <span class="text-sm md:text-lg glow-white menu-item"
                    @click="router.push({ name: 'emoji-category-admin' })">表情分类管理</span>
            </div>
            <div id="right" class="flex-1 h-full flex flex-col min-w-0">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center">
                        <el-icon v-if="white_page" class="mr-4 cursor-pointer" size="24" color="white"
                            @click="toggleTheme">
                            <Moon />
                        </el-icon>
                        <el-icon v-if="!white_page" class="mr-4 cursor-pointer" size="24" color="yellow"
                            @click="toggleTheme">
                            <Sunny />
                        </el-icon>
...
.el-table {
    ...
    --el-table-tr-bg-color: rgba(255, 255, 255, 0.2) !important;
...
.el-table th.el-table__cell {
    --el-table-tr-bg-color: rgba(255, 255, 255, 0.2) !important;
    --el-table-bg-color: rgba(255, 255, 255, 0.2) backdrop-filter: blur(10px);
